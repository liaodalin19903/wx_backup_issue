/**
 * Created by ldl on 2018/3/17.
 */

/*
 本地localStorage封装 API
 localStorage存储的值都会转换为字符串
 */
...
/*
 ...
 set(name, value, time)		// 设置某条缓存 (time格式: 设置天数: 'day:2' 或者 设置分钟:'min:2'  time为可选参数，默认会设置在 1 day 后过期)
 get(name)			// 取某条缓存
 recycle()				// 删除过期缓存  - 页面加载后自动运行
 clear()				// 清空缓存(！慎用)
 remove(name)		// 删除某条缓存
 */
...
export default window.myStorage = (function(win,dom){
  ...
  // 设置缓存时判断传入值，把传入值变成JSON.stringfy存储
  ...
  function isDated(storageItem){
    ...
    var itemArr=String(storageItem).split(':-:');
